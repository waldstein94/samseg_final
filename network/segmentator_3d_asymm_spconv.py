...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        self.down_seg = nn.Sequential(
            nn.Linear(num_seg_features, 2*init_size) 
        )
        ...
        self.secon_compress0 = spconv.SubMConv3d(4 * 3 * init_size, 3 * init_size, indice_key="second_compress", kernel_size=3,
                                                 stride=1, padding=1,bias=True)
        self.secon_compress1 = spconv.SubMConv3d(3 * init_size, 64, indice_key="second_compress", kernel_size=3,
                                                 stride=1, padding=1,bias=True)
        #####
    ...
    def forward(self, voxel_features, coors, batch_size, pooled):
        ...
        ####
        mid_sp = up1e.replace_feature(torch.cat((up1e.features, feat_seg, feat_pix), 1))
        ...
        scomp0 = self.secon_compress0(sup1e)
        scomp1 = self.secon_compress1(scomp0)
        #####
        # breakpoint()
        up0e = self.ReconNet(scomp1)
        up0e = up0e.replace_feature(torch.cat((up0e.features, up1e.features), 1))
        logits = self.logits(up0e)
        logits = logits.dense()
        
        up1e_ins_heatmap = scomp1
        ...
        return logits, heatmap, offset, instmap, scomp1.features
