...
class Asymm_3d_spconv(nn.Module):
    ...
    def forward(self, voxel_features, coors, batch_size, pooled):
        ...
        down_seg, down_seg2 = self.resSeg(ret_seg)
        down_pix, down_pix2 = self.resPix(ret_pix)
        
        up2e = up2e.replace_feature(torch.cat((up2e.features, down_seg.features, down_pix.features), 1))
        down1b = down1b + down_seg2 + down_pix2
        # mid_sp = up1e.replace_feature(torch.cat((up1e.features, feat_pix), 1))
        
        scomp0 = self.secon_compress0(up2e)
