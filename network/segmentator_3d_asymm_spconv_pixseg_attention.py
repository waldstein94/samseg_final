...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        # gh
        self.fea_attn = torch.nn.MultiheadAttention(embed_dim=64, num_heads=4, dropout=0.1,
                                                    batch_first=True)
    ...
    def forward(self, voxel_features, coors, batch_size, pooled):
        ...
        up2e = self.upBlock2(up3e, down2b)  # [2, 128, 240, 180, 16]
        ...
        down_seg, down_seg2 = self.resSeg(ret_seg)  # [2, 64, 240, 180, 16]
        down_pix, down_pix2 = self.resPix(ret_pix)  # [2, 64, 240, 180, 16]

        # breakpoint()

        # gh attention based fusion
        # up2e = up2e.replace_feature(torch.cat((up2e.features, down_seg.features, down_pix.features), 1))  # 21569, 256  [21569, 384])
        up2e = up2e.replace_feature(self.down_geo(up2e.features))
        cat_fea = torch.cat((up2e.features[:,None,:], down_seg.features[:,None,:], down_pix.features[:,None,:]), 1)
        attn_fea = self.fea_attn(cat_fea, cat_fea, cat_fea)[0]
        up2e = up2e.replace_feature(attn_fea.reshape((-1, 64*3)))
        down1b = down1b + down_seg2 + down_pix2
        # mid_sp = up1e.replace_feature(torch.cat((up1e.features, feat_pix), 1))


        scomp0 = self.secon_compress0(up2e)  # ([2, 128, 240, 180, 16])
        
        
        # sdown1c, sdown1b = self.seconBlock0(mid_sp) # 192 -> 384
        # sdown2c, sdown2b = self.seconBlock1(sdown1c) # 384 -> 768

        # sup2e = self.seconup0(sdown2c, sdown2b) # 
        # sup1e = self.seconup1(sup2e, sdown1b) # 384
        
        # scomp1 = self.secon_compress1(up2e)
        #####
        # breakpoint()
        
        up1e = self.upBlock3(scomp0, down1b)
